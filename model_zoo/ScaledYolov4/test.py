...
def test(data,
    ...
    if training:  # called by train.py
        ...
        save_dir = os.path.join(weights[0].split("/")[3],weights[0].split("/")[4])+"/test"
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        # Remove previous
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            if save_txt:
                ...
                predd = torch.zeros_like(pred)
                predd.copy_(pred)
                predd[:, :4] = scale_coords(img[si].shape[1:], predd[:, :4], shapes[si][0], shapes[si][1])  # to original
                for *xyxy, conf, cls in predd:
                    xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                    with open(txt_path + '.txt', 'a') as f:
                        f.write(('%g ' * 6 + '\n') % (conf, cls, *xywh))  # label format
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-txt', default=1, action='store_true', help='save results to *.txt')
